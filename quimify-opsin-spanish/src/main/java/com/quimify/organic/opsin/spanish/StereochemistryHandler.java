...
import com.quimify.organic.opsin.spanish.BondStereo.BondStereoValue;
import com.quimify.organic.opsin.spanish.OpsinWarning.OpsinWarningType;
import com.quimify.organic.opsin.spanish.StereoAnalyser.StereoBond;
import com.quimify.organic.opsin.spanish.StereoAnalyser.StereoCentre;
import static com.quimify.organic.opsin.spanish.XmlDeclarations.*;
	...
	private void assignStereoCentre(Element stereoChemistryEl) throws StructureBuildingException, StereochemistryException {
		...
		if (possibleWordParent.getName().equals(WORD_EL) && possibleWordParent.getChild(0).equals(parentSubBracketOrRoot)){
			...
			for (Element word : words) {
				List<Element> possibleGroups = OpsinTools.getDescendantElementsWithTagName(word, GROUP_EL);
				for (int i = possibleGroups.size()-1; i >=0; i--) {
					if (attemptAssignmentOfStereoCentreToFragment(possibleGroups.get(i).getFrag(), rOrS, locant, grpType)) {
						return;
					}
				}
			}
